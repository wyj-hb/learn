...
% 这时候可以用到矩阵与常数的大小判断运算
D == Max_eig%除了元素值等于Max_eig的位置为1，其余均为0
[r,c] = find(D == Max_eig , 1)%返回第一个符合要求的元素的下标
% 找到D中第一个与最大特征值相等的元素的位置，记录它的行和列。

% 第二步：对求出的特征向量进行归一化即可得到我们的权重
V(:,c)%得到的是最大特征值对应的特征向量
disp('特征值法求权重的结果为：');
disp( V(:,c) ./ sum(V(:,c)) )
...
clc
CI = (Max_eig - n) / (n-1);%CI等于最大特征值-n/n-1;
